...
| M5 Publishing & subdomains | Slugs + reserved list, publish route, subdomain rewrite, published renderer, SEO meta | `lib/slug.ts`, `lib/slug-format.ts`, `app/api/sites/[id]/publish/route.ts`, `middleware.ts`, `app/sites/[slug]/page.tsx` |
...
- The "37/37 unit tests" in those docs came from `scripts/test-slug-edge-cases.mjs`, which copied slug logic instead of importing `lib/slug.ts`. It has been replaced by `tests/unit/slug.test.ts` (`npm run test:edges` now runs that suite). The unit tests surfaced suspected bugs. Both were fixed on 2026-09-25 (owner-approved): the Agency 1-site limit, and slug generation (accent transliteration, `[a-z0-9-]` only, 63-char cap, non-empty fallback). See [NEXT_ACTIONS.md](NEXT_ACTIONS.md) item 8.
