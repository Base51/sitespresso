...
Assert-FileContains "lib/sites/create-draft.ts" "isSiteLimitReached" "Draft creation enforces per-plan site limits (server-side)"
Assert-FileContains "components/SitePreview.tsx" "/api/sites" "Draft creation goes through the server route"
...
Assert-FileContains "app/api/referrals/route.ts" ".like('referral_code'" "Referral attribution API resolves indexed referral codes"
...
Assert-PathExists "supabase/migrations/20260905193000_add_leads_and_referrals.sql" "Lead and referral schema migration exists"
