...
- There is no unit-test framework yet (see [NEXT_ACTIONS.md](NEXT_ACTIONS.md) item 2).
...
| `build-verify.yml` (Build Verification) | `npm ci`, `npx tsc --noEmit`, `npm run lint`, `npm run build`, checks `.next/static` exists |
...
| Scripts | `scripts/*.ps1` (pwsh), `scripts/*.ts` (tsx), `scripts/test-slug-edge-cases.mjs` |
